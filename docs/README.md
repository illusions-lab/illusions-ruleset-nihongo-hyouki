...
3. `docs/rules/<ruleId>.md` を [docs/rules/nh-katakana-wi-we-wo.md](./rules/nh-katakana-wi-we-wo.md) のテンプレートに
   従って作成する（**ルールごとに1ファイル**）。
...
- `docs/rules/` のファイル名は `ruleId` と一致させる（例 `nh-katakana-wi-we-wo` → `docs/rules/nh-katakana-wi-we-wo.md`）。
